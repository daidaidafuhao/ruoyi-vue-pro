...
public class dronesServiceImpl implements dronesService {
    ...
    public void updatedrones(dronesSaveReqVO updateReqVO) {
        // 校验存在
        validatedronesExists(updateReqVO.getId());
        // 更新
    ...
    @Override
    public void deletedrones(String id) {
        // 校验存在
        validatedronesExists(id);
        // 删除
        dronesMapper.deleteById(id);
    }

    @Override
        public void deletedronesListByIds(List<String> ids) {
        // 校验存在
        validatedronesExists(ids);
        // 删除
        dronesMapper.deleteByIds(ids);
        }

    private void validatedronesExists(List<String> ids) {
        List<dronesDO> list = dronesMapper.selectByIds(ids);
        if (CollUtil.isEmpty(list) || list.size() != ids.size()) {
            throw exception(DRONES_NOT_EXISTS);
        }
    }

    private void validatedronesExists(String id) {
        if (dronesMapper.selectById(id) == null) {
            throw exception(DRONES_NOT_EXISTS);
        }
    }

    @Override
    public dronesDO getdrones(String id) {
        return dronesMapper.selectById(id);
    }
